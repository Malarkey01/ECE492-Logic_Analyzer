...
class SerialWorker(QThread):
    ...
    def __init__(self, port, baudrate, channels=8):
        ...
        self.trigger_modes = ['No Trigger'] * 4  # 4 groups
        try:
    ...
    def set_trigger_mode(self, group_idx, mode):
        self.trigger_modes[group_idx] = mode

    def run(self):
        pre_trigger_buffer_size = 1000
        data_buffer = []
        triggered = [False] * 4  # 4 groups
                        ...
                        for i in range(4):
                            if not triggered[i] and self.trigger_modes[i] != 'No Trigger':
                                ...
                                if last_value is not None:
                                    current_bits = ((data_value >> (2 * i)) & 0b11)
                                    last_bits = ((last_value >> (2 * i)) & 0b11)

                                    # For simplicity, consider trigger when any bit changes
                                    if self.trigger_modes[i] == 'Rising Edge' and last_bits < current_bits:
                                        triggered[i] = True
                                        print(f"Trigger condition met on group {i+1}: Rising Edge")
                                    elif self.trigger_modes[i] == 'Falling Edge' and last_bits > current_bits:
                                        triggered[i] = True
                                        print(f"Trigger condition met on group {i+1}: Falling Edge")
...
class I2CDisplay(QWidget):
    def __init__(self, port, baudrate, channels=8):
        ...
        self.data_buffer = [[] for _ in range(4)]  # 4 groups
        self.channel_visibility = [False] * 4  # Only 4 buttons

        self.is_single_capture = False  # Initialize single capture flag

        # Initialize trigger modes per group
        self.current_trigger_modes = ['No Trigger'] * 4

        self.trigger_mode_indices = [0] * 4  # Indices in ['No Trigger', 'Rising Edge', 'Falling Edge']
    ...
    def setup_ui(self):
        ...
        self.plot.setYRange(-2, 2 * 4, padding=0)  # Only 4 channels displayed
        ...
        self.curves = []
        for i in range(4):  # Only 4 curves
        ...
        button_layout.addLayout(control_buttons_layout, 6, 0, 1, 2)
    ...
    def toggle_trigger_mode(self, group_idx):
        self.trigger_mode_indices[group_idx] = (self.trigger_mode_indices[group_idx] + 1) % len(self.trigger_mode_options)
        mode = self.trigger_mode_options[self.trigger_mode_indices[group_idx]]
        self.trigger_mode_buttons[group_idx].setText(mode)
        self.current_trigger_modes[group_idx] = mode  # Update current trigger mode
        if self.worker:
            self.worker.set_trigger_mode(group_idx, mode)
    ...
    def toggle_channel(self, group_idx, is_checked):
        self.channel_visibility[group_idx] = is_checked
        self.curves[group_idx].setVisible(is_checked)
    ...
    def handle_data(self, data_list):
        if self.is_reading:
            for data_value in data_list:
                for i in range(4):
                    # Combine two channels for each group
                    bit1 = (data_value >> (2 * i)) & 1
                    bit2 = (data_value >> (2 * i + 1)) & 1
                    combined_value = (bit1 << 1) | bit2  # Combine bits into a 2-bit value
                    self.data_buffer[i].append(combined_value)
    ...
    def update_cursor_position(self):
        ...
        self.cursor_label.setPos(cursor_pos, 4 * 2 - 1)
