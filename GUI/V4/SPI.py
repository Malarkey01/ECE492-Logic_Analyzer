...
class SerialWorker(QThread):
    ...
    def __init__(self, port, baudrate, channels=8):
        ...
        self.trigger_modes = ['No Trigger'] * 2  # 2 groups
        try:
                        ...
                        for i in range(2):
                            if not triggered[i] and self.trigger_modes[i] != 'No Trigger':
                                ...
                                if last_value is not None:
                                    # Combine four bits for each group
                                    current_bits = ((data_value >> (4 * i)) & 0b1111)
                                    last_bits = ((last_value >> (4 * i)) & 0b1111)

                                    # Trigger condition based on combined bits
...
class SPIDisplay(QWidget):
    def __init__(self, port, baudrate, channels=8):
        ...
        self.data_buffer = [[] for _ in range(2)]  # 2 groups
        self.channel_visibility = [False] * 2  # Only 2 buttons

        self.is_single_capture = False  # Initialize single capture flag

        # Initialize trigger modes per group
        self.current_trigger_modes = ['No Trigger'] * 2

        self.trigger_mode_indices = [0] * 2  # Indices in ['No Trigger', 'Rising Edge', 'Falling Edge']
    ...
    def setup_ui(self):
        ...
        self.plot.setYRange(-2, 2 * 2, padding=0)  # Only 2 channels displayed
        ...
        self.colors = ['#FF6EC7', '#39FF14']
        self.curves = []
        for i in range(2):  # Only 2 curves
        ...
        button_layout.addLayout(control_buttons_layout, 4, 0, 1, 2)
    ...
    def handle_data(self, data_list):
        if self.is_reading:
            for data_value in data_list:
                for i in range(2):
                    # Combine four channels for each group
                    bits = (data_value >> (4 * i)) & 0b1111  # Extract 4 bits
                    self.data_buffer[i].append(bits)
    ...
    def update_cursor_position(self):
        ...
        self.cursor_label.setPos(cursor_pos, 2 * 2 - 1)
